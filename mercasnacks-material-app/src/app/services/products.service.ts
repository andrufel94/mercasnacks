...
export class ProductService {
    ...
    getProductsByUrl() {
        ...
        return this._http.get(this.url, { headers: headers }).pipe(map(res => this.transformProducts(res.json().retailer.products)));
    }
    ...
    //  ****************************************************
    //  Author: Andres Luque    Update: 2018/11/03
    //  Method: Delete a product into shopping cart
    //  ****************************************************
    deleteProductCart(product: Product) {
        let aProductsCart: Product[];
        aProductsCart = this.getProductsCart();
        let aProductsCartFilter = aProductsCart.filter( (productFilter) => productFilter._id !== product._id);

        localStorage.setItem('aProductsCart', JSON.stringify(aProductsCartFilter));
        return aProductsCart;
    }
}